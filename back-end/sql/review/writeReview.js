...
function getWritableReviews(memberId, callback) {
  const query = `
    SELECT oi.product_id, p.p_image_url, p.p_name, c.color_name, s.size
    FROM order_items oi
    ...
    WHERE o.member_id = ? AND NOT EXISTS (
      SELECT 1
      FROM reviews r
      WHERE r.member_id = o.member_id AND r.product_id = oi.product_id
    )
  `;
...
function getWrittenReviews(memberId, callback) {
  const query = `
    SELECT r.review_id, r.rating, r.review_text, p.p_image_url, p.p_name, c.color_name, s.size
    FROM reviews r
